...
export default function App() {
    ...
    const BASE_URL = 'https://becc-34-125-201-143.ngrok-free.app';
    ...
    const startGeneration = (retryStage?: number) => {
        ...
        const websocket = new WebSocket(`${BASE_URL.replace('http', 'ws')}/ws/generate`);
        ...
        websocket.onmessage = async (event) => {
            ...
            if ((data.status === 'svg_ready' || data.status === 'svg_ready2' || data.status === 'svg_ready3') && data.svg_file) {
                try {
                    const response = await fetch(`${BASE_URL}/api/svg/${data.svg_file}`);
